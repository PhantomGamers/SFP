...
internal readonly struct EntryStore
{
    ...
    [SuppressMessage("CodeSmell", "ERP022:Unobserved exception in generic exception handler",
        Justification = "Can error with corrupted cache")]
...
internal readonly struct Addr
{
    ...
    public Addr(uint address, string directory)
    {
        ...
        {
            fileName = $"f_{Convert.ToInt32(BinaryString[4..], 2):x6}";
        }
        else
        {
    ...
    private static int BlockSizeForFileType(FileType fileType) => fileType switch
    {
