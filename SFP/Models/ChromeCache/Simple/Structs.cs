...
public readonly struct SimpleFileHeader
{
    ...
    public IEnumerable<byte> Serialize() => Utils.StructureToByteArray(this);
}
...
public struct SimpleFileEof
{
    ...
    public readonly IEnumerable<byte> Serialize() => Utils.StructureToByteArray(this);

    public readonly bool HasSHA256() => (flags & (uint)Flags.FLAG_HAS_KEY_SHA256) == (uint)Flags.FLAG_HAS_KEY_SHA256;
...
public struct SimpleFile
{
    ...
    public SimpleFile(SimpleFileHeader header, string key, byte[] eof0_data, byte[] eof1_data, SimpleFileEof eof0,
        SimpleFileEof eof1, byte[] sha256, FileInfo file)
    {
        _header = header;
        _key = key;
