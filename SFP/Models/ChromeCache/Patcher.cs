...
namespace SFP.Models.ChromeCache
{
    public class Patcher
    {
        ...
        public static async Task<(byte[], bool)> PatchFriendsCSS(byte[] data, string fileName)
        {
            ...
            var bytes = await UtilsModel.DecompressBytes(data);

            var patchedTextBytes = Encoding.UTF8.GetBytes(LocalFileModel.PATCHED_TEXT);
            ...
            var append = Encoding.UTF8.GetBytes(appendText);
            bytes = append.Concat(bytes).Concat(Encoding.UTF8.GetBytes("}")).ToArray();

            var customFile = Path.Join(SteamModel.ClientUIDir, "friends.custom.css");
